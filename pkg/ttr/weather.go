...
func getWeather(location string, botUnits string, hoursFromNow int) (string, string, string, string, string, string) {
	...
	weatherAPIKey := os.Getenv(vars.WeatherAPIKeyEnv)
	if weatherAPIKey == "" {
		fmt.Println("weather API key not existant")
	}
	weatherAPIUnit := "F"
		...
		if weatherAPIProvider == "weatherapi.com" {
			...
			for _, b := range weatherAPICladMap {
				if b.APIValue == weatherStruct.Current.Condition.Text {
					...
					fmt.Println("API Value: " + b.APIValue + ", Clad Type: " + b.CladType)
					matchedValue = true
					break
				}
			}
			...
			if err != nil {
				fmt.Println(err)
				fmt.Println("Geolocation API error: " + geoCodingResponse)
			}
			if len(geoCodingInfoStruct) == 0 {
				fmt.Println("Geo provided no response.")
				condition = "undefined"
				is_forecast = "false"
			...
			fmt.Println("Name: " + geoCodingInfoStruct[0].Name)
			fmt.Println("Country: " + geoCodingInfoStruct[0].Country)
...
func weatherParser(speechText string, botLocation string, botUnits string) (string, string, string, string, string, string) {
	...
	fmt.Println("Looking for forecast " + strconv.Itoa(hoursFromNow) + " hours from now...")
